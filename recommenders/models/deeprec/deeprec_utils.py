...
def gen_fatigue_features(cate_history, cate_target, time_to_now):
    thrs = [3600, 3600*3, 3600*24]
    feat = []
    for thr in thrs:
        tmp = cate_history[np.where(time_to_now<=thr)[0]]
        feat.extend([len(tmp), np.sum(tmp==cate_target)])
    return feat
...
def cal_metric(users, labels, preds, metrics):
    """Calculate metrics.
