...
class BaseModel:
    ...
    def _get_loss(self):
        ...
        if self.our_model:
            pred_fatigue_logit = tf.concat([tf.reshape(self.fatigue_logit[self.iterator.labels>0], [-1, 1]), \
                                            tf.reshape(self.fatigue_logit_fatigue, [-1, self.train_num_ngs])], -1)
            softmax_pred = tf.nn.softmax(-pred_fatigue_logit, axis=-1)
            group_labels = tf.reshape(self.iterator.labels, (-1, self.train_num_ngs+1))
            boolean_mask = tf.equal(group_labels, tf.ones_like(group_labels))
            mask_paddings = tf.ones_like(softmax_pred)
            pos_softmax = tf.compat.v1.where(boolean_mask, softmax_pred, mask_paddings)
            self.fatigue_loss = tf.reduce_sum(-self.iterator.CL_mask[self.iterator.labels[:, 0]>0] * tf.reduce_sum(tf.math.log(pos_softmax), -1, keepdims=True))/tf.reduce_sum(self.iterator.CL_mask[self.iterator.labels[:, 0]>0])
            self.loss = tf.add(self.loss, self.hparams.alpha*self.fatigue_loss)
    ...
    def _add_summaries(self):
        ...
        if self.our_model:
            tf.compat.v1.summary.scalar("fatigue_loss", self.fatigue_loss)
    ...
    def train(self, sess, feed_dict):
        ...
        if self.our_model:
            # to_run += [self.fatigue_loss, self.fatigue_logit, self.fatigue_logit_fatigue]
            to_run += [self.fatigue_loss]
        return sess.run(to_run,
            feed_dict=feed_dict,
        )
    ...
    def infer(self, sess, feed_dict):
        ...
        feed_dict[self.layer_keeps] = self.keep_prob_test
        feed_dict[self.is_train_stage] = False
        # return sess.run([self.logit, self.fatigue_logit, self.recent_sim], feed_dict=feed_dict)
        return sess.run([self.logit], feed_dict=feed_dict)
    ...
    def _fcn_net(self, model_output, layer_sizes, scope, activations=None, BN=None):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            with tf.compat.v1.variable_scope(
                "nn_part", initializer=self.initializer, reuse=tf.compat.v1.AUTO_REUSE
            ) as scope:
