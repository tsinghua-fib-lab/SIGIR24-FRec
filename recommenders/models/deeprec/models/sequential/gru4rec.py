...
class GRU4RecModel(SequentialBaseModel):
    ...
    def _build_seq_graph(self):
        ...
        """
        with tf.compat.v1.variable_scope("gru4rec", initializer=self.initializer):
    ...
    def _build_gru(self):
        ...
        with tf.compat.v1.name_scope("gru"):
            ...
            rnn_outputs, final_state = dynamic_rnn(
                GRUCell(self.hidden_size, kernel_initializer=self.initializer, bias_initializer=self.initializer),
                inputs=self.history_embedding,
                sequence_length=self.sequence_length,
                dtype=tf.float32,
                scope="gru",
