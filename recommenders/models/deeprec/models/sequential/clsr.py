...
class CLSRModel(SequentialBaseModel):
    ...
    def _lookup_from_embedding(self):
        ...
        tf.compat.v1.summary.histogram("user_long_embedding_output", self.user_long_embedding)

        self.user_short_embedding = tf.nn.embedding_lookup(
            self.user_short_lookup, self.iterator.users
        )
        tf.compat.v1.summary.histogram("user_short_embedding_output", self.user_short_embedding)
    ...
    def _build_seq_graph(self):
        ...
        with tf.compat.v1.variable_scope("clsr"):
            ...
            with tf.compat.v1.variable_scope("long_term"):
                ...
                tf.compat.v1.summary.histogram("att_fea_long", self.att_fea_long)
            ...
            with tf.compat.v1.variable_scope("short_term"):
                if hparams.interest_evolve:
                    _, short_term_intention = dynamic_rnn(
                        GRUCell(hparams.user_embedding_dim, kernel_initializer=self.initializer, bias_initializer=self.initializer),
                        inputs=hist_input,
                ...
                tf.compat.v1.summary.histogram("GRU_final_state", short_term_intention)
                ...
                if hparams.sequential_model == 'time4lstm':
                    ...
                    rnn_outputs, _ = dynamic_rnn(
                        Time4LSTMCell(hparams.hidden_size, initializer=self.initializer),
                    ...
                    rnn_outputs, _ = dynamic_rnn(
                        tf.nn.rnn_cell.LSTMCell(hparams.hidden_size, initializer=self.initializer),
                        inputs=hist_input,
                        sequence_length=self.sequence_length,
                        dtype=tf.float32,
                        scope="simple_lstm",
                ...
                short_term_query = short_term_intention
                # tf.concat([short_term_intention, self.target_item_embedding], -1)
                att_outputs_short = self._attention_fcn(short_term_query, rnn_outputs)
                self.att_fea_short = tf.reduce_sum(att_outputs_short, 1)
                tf.compat.v1.summary.histogram("att_fea2", self.att_fea_short)
                ...
                if not hparams.manual_alpha:
                    if hparams.predict_long_short:
                        with tf.compat.v1.variable_scope("causal2"):
                            ...
                            tf.compat.v1.summary.histogram("causal2", final_state)
                    ...
                    tf.compat.v1.summary.histogram("alpha", self.alpha_output)
                    # self.alpha_output_mean = self.alpha_output
                    # error_with_category = self.alpha_output_mean - self.iterator.attn_labels
                    # tf.compat.v1.summary.histogram("error_with_category", error_with_category)
                    ...
                    # tf.compat.v1.summary.histogram("squared_error_with_category", squared_error_with_category)
                else:
    ...
    def _attention_fcn(self, query, user_embedding):
        ...
        with tf.compat.v1.variable_scope("attention_fcn"):
            ...
            # )
            queries = tf.tile(tf.expand_dims(query, 1), [1, get_shape(att_inputs)[1], 1])
    ...
    #     tf.compat.v1.summary.scalar("contrastive_loss", self.contrastive_loss)
    #     tf.compat.v1.summary.scalar("discrepancy_loss", self.discrepancy_loss)
